...
function GetLeadingWhite(const aStr: UnicodeString): UnicodeString;
...
begin
  ...
  I := 1;
  while (I <= Length(aStr)) and (aStr[I] < #33) do
  begin
...
begin
  ...
  if (ALine < 1) or (ALine > (ASynEdit as TCustomSynEdit).Lines.Count) then
    Exit;
  S := (ASynEdit as TCustomSynEdit).Lines.List^[Pred(ALine)].fString;
  for I := 1 to Length(S) do
    if S[I] > #32 then
    begin
      Result := False;
...
begin
  Result := 0;
  if (ALine < 1) or (ALine > (ASynEdit as TSynEdit).Lines.Count) then
    Exit;
  S := (ASynEdit as TSynEdit).Lines.List^[Pred(ALine)].fString;
  for I := 1 to Length(S) do
    if S[I] > #32 then
      Break
    else
