...
// use scAll to update a statusbar when another TCustomSynEdit got the focus
  ...
  protected
    { do not define this methods as abstract because they have to be called while
      ancestors may not have an implementation of them }
...
{$ENDIF}
  private
    ...
    FActiveLineColor: TSynSelectedColor;
...
{$ENDIF}
    ...
    procedure ActiveLineColorsChanged(Sender: TObject);
...
{$ENDIF}
  ...
  public
    ...
    property ActiveLineColor: TSynSelectedColor read FActiveLineColor
      write FActiveLineColor;
    ...
    property LastMatch: TSynTokenMatched read FLastMatch;
...
{$ENDIF}
  { SynEdit }
  SynUniHighlighter, SynTokenMatch, SynUniRules,
  { TBX }
  TBXUtils;
...
{$ENDIF}
  ...
  Result.Column := Max(1, FLeftChar + Round((aX - FGutterWidth - 2) / FCharWidth));
  Result.Row := Max(1, FTopLine + (aY div FTextHeight));
end;
...
{$ENDIF}
  Result.Column := Max(1, FLeftChar + ((aX - FGutterWidth - 2) div FCharWidth));
  Result.Row := Max(1, FTopLine + (aY div FTextHeight));
end;
...
{$ENDIF}
  ...
  FActiveLineColor := TSynSelectedColor.Create;
  FActiveLineColor.Background := clNone;
  FActiveLineColor.Foreground := clNone;
  FActiveLineColor.OnChange := ActiveLineColorsChanged;
  ...
  { Gutter mast be initialized strongly after FTextDrawer initialization }
  FGutter := TSynGutter.Create;
  FGutter.OnChange := GutterChanged;
  FGutterWidth := GetGutterWidth();
  ...
  FOptions := TSynEditorOptions.Create();
    ...
    // Repaint according to specified bounds
      ...
      begin
        ...
        rcInval := Rect(0, FTextHeight * (cFirstRow - FTopLine),
          FGutterWidth, FTextHeight * (cLastRow - FTopLine + 1));
{$IFDEF SYN_CLX}
        with GetClientRect do
          OffsetRect(rcInval, Left, Top);
{$ENDIF}
...
begin
  if Visible and HandleAllocated then
    ...
    // Repaint according to specified bounds
      ...
      begin
        // Calculate repaint rect
        rcInval := Rect(FGutterWidth, FTextHeight * (cFirstRow - FTopLine),
...
procedure TCustomSynEdit.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  bWasSel, bStartDrag, bCanLineSelect: Boolean;
...
begin
  ...
  if FRightEdgeShow and (FRightEdge > 0) then
    if (Button = mbLeft) and (ssCtrl in Shift) and (Abs(RowColumnToPixels(DisplayCoord(
  ...
  begin
    ...
    FBlockEnd := BufferCoord(MaxScrollWidth, FCaretY);
    ActiveSelectionMode := smLine;
{$MESSAGE WARN 'Is it really necessary to call InvalidateSelection after SetActiveSelectionMode?'}
    InvalidateSelection;
    Exit;
  end;
  ...
  begin
    ...
    // the block positioning is lost. This makes sure that the block is maintained
    // in case they started a drag operation on the block
...
{$ENDIF}
  ...
  if (rcClip.Left > FGutterWidth + 2) then
    Inc(nC1, (rcClip.Left - FGutterWidth - 2) div FCharWidth);
  nC2 := LeftChar +
    (rcClip.Right - FGutterWidth - 2 + FCharWidth - 1) div FCharWidth;

  // Rows
  nL1 := Max(FTopLine + rcClip.Top div FTextHeight, TopLine);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          FTextDrawer.ExtTextOut(rcLine.Left, rcLine.Top, [tooOpaque], rcLine,
            '', 0);
        end;
        // Next line rect
        rcLine.Top := vLineTop;
...
{$IFDEF SYN_WIN32}
  // draw word wrap glyphs transparently otherwise you'll see glyph alpha color
  // flashing while repainting without DoubleBuffering
  if FGutter.Gradient then
    Canvas.Brush.Style := bsClear;
{$ENDIF}
...
var
  ...
  bFirstRow, bSpecialLine, bCurrentLine,
  bLineSelected, bTokenSelected: Boolean;
  colFG, colBG: TColor;
...
{$ENDIF}
  ...
  begin
    jAmount := -9;
    if (eoAdaptiveSpecialLine in Options) and (bCurrentLine) then
    ...
    else if (ActiveLineColor.Background <> clNone) and (bCurrentLine) then
      Result := ActiveLineColor.Background
    else begin
      Result := Color;
      if Highlight then
      begin
  ...
  begin
    ...
    with FTextDrawer do
      ...
      begin
        ...
        if Invert then
          BGColor := not BGColor and $00FFFFFF;
...
{$ENDIF}
  ...
  var
    ...
    begin
      ...
      for I := 0 to High(FCarets) do
      begin
        // Find out caret position intersection
        if (nLine = FCarets[I].bcCaret.Line) and InRange(FCarets[I].bcCaret.Char,
          Succ(TokenAccu.Pos), Succ(TokenAccu.Pos) + n4) then
        begin
          J := CharToPixels(fCarets[I].bcCaret.Char);
          if (J > -1) and (FCarets[I].bcCaret.Char = Succ(Lines.AccessStringLength(
            Pred(FCarets[I].bcCaret.Line)))) then
        ...
        if (nLine = FCarets[I].bcStart.Line) and (FCarets[I].bcStart.Char >= Succ(TokenAccu.Pos))
          and (FCarets[I].bcStart.Char <= Succ(TokenAccu.Pos) + n4) then
        ...
        // Selection block inside token (bcEnd check)
          ...
          and (FCarets[I].bcEnd.Char <= Succ(TokenAccu.Pos) + n4) then
        ...
        begin
          ...
          if nLine = FCarets[I].bcStart.Line then
            ...
            begin
              ...
              (Succ(TokenAccu.Pos) + n4 < FCarets[I].bcEnd.Char) then
              Whole := True
            else
  ...
  begin
    ...
    nC2 := Min(LastCol, TokenAccu.CharsBefore + TokenAccu.ExpandedLen + 1);
    nC3 := 1;
    nC4 := TokenAccu.Len;
...
{$ENDIF}
      ...
      begin
        if (eoShowSpecialcharsInSelection in FOptions)
          and not (eoShowSpecialchars in FOptions)
          and not (bCanPaintEolMark) then
          Exit;
...
//            Pen.Color := FOutliningAttr.Foreground;
  ...
  procedure AddHighlightToken(const Token: UnicodeString;
    CharsBefore, TokenLen, TokenExpandedLen: Integer;
    Foreground, Background: TColor; Style: TFontStyles; TokenPos: Integer);
  var
    bCanAppend: Boolean;
  ...
  begin
    ...
    // Fill with new chars
      ...
      TokenAccu.ExpandedLen := TokenExpandedLen;
  ...
  var
    ...
    nTokenPos, nTokenLen, nOldTokenLen: Integer;
      ...
      begin
        ...
        begin
          ...
          if eoAlignedWrap in FOptions then
            if not bFirstRow and (nIndent = 0) then
              nIndent := GetLeadingExpandedLength(Lines.List^[nLine - 1].FString,
              FTabWidth);
          ...
          begin
            ...
            begin
              if (FActiveSelectionMode = smColumn) then
                nLineSelStart := BufferToDisplayPos(DisplayToBufferPos(
                  DisplayCoord(vSelStart.Column, cRow))).Column
              else
        ...
        // Paint with grammar
          ...
          begin
            ...
            nTokenLen := ExpandedPos(FHighlighter.GetExpandedTokenPos,
              FHighlighter.GetExpandedTokenPos + FHighlighter.GetExpandedTokenLen,
              StrA);
...
//              Dec(nTokenPos, vFirstChar - FirstCol);
              ...
              if Assigned(attr) then
                AddHighlightToken(sToken, nIndent + nTokenPos - (vFirstChar - FirstCol),
                  FHighlighter.GetExpandedTokenLen, nTokenLen, Attr.Foreground,
                  Attr.Background, Attr.Style, FHighlighter.GetExpandedTokenPos)
              else
                AddHighlightToken(sToken, nIndent + nTokenPos - (vFirstChar - FirstCol),
                  FHighlighter.GetExpandedTokenLen, nTokenLen, colFG, colBG, Font.Style,
                  FHighlighter.GetExpandedTokenPos);
            end;
          ...
          begin
            if WordWrap then
              if (cRow = vEndRow) and (Lines.Count > 0) then
                PaintHighlightToken(True,
                  FWordWrapPlugin.GetRowLength(vEndRow, nLine) + 1 + nIndent)
              else
                PaintHighlightToken(True)
            else if not WordWrap then
...
procedure TCustomSynEdit.SelectCurrentScope;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SetLine(FLines.List^[Pred(nLineEnd)].FString, Pred(nLineEnd));
        end;
      end;
      ...
      begin
        ...
        SetLine(FLines.List^[Pred(nLineStart)].FString, Pred(nLineStart));
        { Still on scope? }
  ...
  begin
    ...
    begin
      ...
      SetLine(FLines.List^[Pred(FCaretY)].FString, Pred(FCaretY));
      { Look behind }
...
function TCustomSynEdit.GetCollapseMarkRect(FoldRange: TSynEditFoldRange; Row,
  Line, Indent: Integer): TRect;
begin
  { Prepare rect }
  with Result do
  begin
  ...
  begin
    ...
    begin
      if FWordWrapPlugin.LineToRow(Line) < Row then
        Inc(Result.Left, GetLeadingExpandedLength(FLines.List^[Pred(Line)].FString, FTabWidth) * FCharWidth);
    end;
  end
...
begin
  ...
  begin
    FTextDrawer.SetBaseFont(FGutter.Font);
    Result := FGutter.GetWidth(FTextDrawer.CharWidth);
    FTextDrawer.SetBaseFont(Font);
  end
  else
    Result := FGutter.GetWidth(FCharWidth);
end;
...
begin
  ...
  if FSelections and (aCaret > -1) then
...
procedure TCustomSynEdit.DoHomeKey(Selection: Boolean);
  ...
  begin
    ...
    begin
      vPos := DisplayXY;
      vPos.Column := Min(CharsInWindow, FWordWrapPlugin.GetRowLength(vPos.Row,
        CaretXY.Line) + 1);
...
procedure TCustomSynEdit.DoEndKey(Selection: Boolean);
  ...
  begin
    ...
    begin
      ...
      while (vLastRow > 1)
        and (FWordWrapPlugin.GetRowLength(vLastRow, CaretY) = 0)
...
begin
  ...
  begin
    ...
    if vEnhance then
      ...
      vNewCaret.Column := FWordWrapPlugin.GetRowLength(vNewCaret.Row, CaretY) + 1;
...
function TCustomSynEdit.FindMatchingPair(var Open, Close: TBufferCoord;
  FoldingOnly: Boolean): Boolean;
var
  nLineStart, nLineEnd, nCharStart, nCharEnd, nLastCheck, nLastCheckStartPos,
  nLastCheckEndPos, nCheck: Integer;
    ...
    begin
        ...
        begin
          ...
          begin
            ...
            if (nLineStart = FCaretY) and (GetTokenPos >= Pred(FCaretX)) then
              Break;
            nCheck := SynTokenMatch.SynEditGetMatchingPartKind(Self,
              Pred(nLineStart), MatchTokens, FoldingOnly);
            if nCheck <> 0 then
            begin
              nCharStart := GetTokenPos;
              nLastCheck := nCheck;
      ...
      begin
        ...
        begin
          ...
          begin
            nCheck := SynTokenMatch.SynEditGetMatchingPartKind(Self,
              Pred(nLineEnd), MatchTokens, FoldingOnly);
            if nCheck <> 0 then
            begin
              nCharEnd := GetTokenPos;
              nLastCheck := nCheck;
...
begin
  ...
  jLine := DisplayToBufferPos(PixelsToRowColumn(FGutterWidth + 2,
...
procedure TCustomSynEdit.OptionsChanged(Sender: TObject);
const
...
begin
{$IFNDEF SYN_CLX}
  bSetDrag := (eoDropFiles in Options) <> (eoDropFiles in Options);
{$ENDIF}
  ...
  bUpdateScroll := (Options * ScrollOptions) <> (Options * ScrollOptions);
...
{$ELSE}
  ...
  if bSetDrag and not (csDesigning in ComponentState) and HandleAllocated then
    DragAcceptFiles(Handle, (eoDropFiles in Options));
{$ENDIF}
  TmpBool := eoShowSpecialChars in Options;
...
procedure TCustomSynEdit.ActiveLineColorsChanged(Sender: TObject);
begin
...
begin
  ...
  FCaretAtEOL := WordWrap and (aPos.Row <= FWordWrapPlugin.GetRowCount) and
    (aPos.Column > FWordWrapPlugin.GetRowLength(aPos.Row, DisplayY)) and
    (DisplayY <> aPos.Row);
  DecPaintLock;
  UpdateLastCaretX;
end;
