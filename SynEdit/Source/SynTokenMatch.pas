      ...
      begin
        ...
        begin
          ...
          begin
            ...
            SetLine(Lines.List^[DeltaLevel].fString, DeltaLevel);
...
begin
  ...
  { Fetch string }
  S := UnicodeStringToUTF8(ASynEdit.Lines.List[Pred(ALine)].fString);
...
var
  ...
  begin
    ...
    begin
      ...
      SetLine(ASynEdit.Lines.List^[Pred(ALine)].fString, Pred(ALine));
...
begin
  ...
  begin
    ...
    S := UnicodeStringToUTF8(ASynEdit.Lines.List[Pred(I)].fString);
...
begin
  ...
  begin
    ...
    bFound := False;
    bOnlyOnce := False;
    S := UnicodeStringToUTF8(ASynEdit.Lines.List^[Pred(APoint.Line)].fString);
...
begin
  ...
  if GetLeadingLength(ASynEdit.Lines.List^[Pred(APoint.Line)].fString) = 0 then
    Exit;

  { Can't unindent if there are some other chars before }
  if not Reindenting then
    if not ASynEdit.IsAllWhiteUpToCaret(ASynEdit.Lines.List^[Pred(APoint.Line)].fString, APoint.Char) then
      Exit;
  ...
  begin
    Open := False;
    Boundary := GetLeadingExpandedLength(ASynEdit.Lines.List^[Pred(APoint.Line)].fString, ASynEdit.TabWidth);
    ...
    begin
      ...
      J := GetLeadingExpandedLength(ASynEdit.Lines.List^[Pred(I)].fString, ASynEdit.TabWidth);
      if J <= Boundary then
      begin
        if J = Boundary then
        begin
