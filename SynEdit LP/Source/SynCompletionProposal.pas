...
uses
  Windows, Messages, Graphics, Forms, Controls, StdCtrls, ExtCtrls, Menus,
  Dialogs, Math, Classes, SysUtils,

  { SynEdit }
  SynEditTypes, SynEditKeyCmds, SynEditHighlighter, SynEditKbdHandler,
  SynEdit, SynUnicode;
...
type
  ...
  private
    ...
    procedure EditorKeyDown(Sender: TObject; var Key: Word; Shift: TSynShiftState);
  ...
  protected
    ...
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
    procedure SetShortCut(Value: TSynShortCut);
  ...
  published
    property ShortCut: TSynShortCut read FShortCut write SetShortCut;
      ...
      begin
        ...
        begin
          if (Length(Parameter) = 2)
            ...
            and CharInSet(WideUpperCase(Parameter[2])[1],
              ['B', 'I', 'U', 'S']) then
          begin
            CommandType := fcStyle;
            ...
            begin
              ...
              PFormatStyleData(Data)^.Style := WideUpperCase(Parameter[2])[1];
              case Parameter[1] of
...
Begin
  ...
  while i <= Length(APrettyText) do
    case APrettyText[i] of
      ...
      #3:
        begin
          if CharInSet(WideUpperCase(APrettyText[i + 1])[1], ['B', 'I', 'U']) then
          begin
            Result := Result + '\style{';
...
begin
  (CurrentEditor as TCustomSynEdit).AlwaysShowCaret := True;
  Visible := True;
  if DisplayType = ctCode then
...
begin
  ...
  (CurrentEditor as TCustomSynEdit).AlwaysShowCaret := False;
end;
  ...
  begin
    ...
    ctCode:
      begin
        BorderWidth := 2 * GetSystemMetrics(SM_CYSIZEFRAME);
        tmpWidth := FWidth;
        tmpHeight := FNbLinesInWindow * Form.FEffectiveItemHeight + BorderWidth;
      end;
    ctHint:
      begin
        BorderWidth := 2;
  ...
  ctCode:
    begin
      ...
      begin
        ...
        {if Form.Owner.Owner is TSynForm then
          ParentForm := GetMDIParent(Form.Owner.Owner as TSynForm)
        else
          ParentForm := nil;
        if Assigned(ParentForm) then
        begin
          ShowWindow(ParentForm.Handle, SW_SHOWNOACTIVATE);
          ShowWindow(Form.Handle, SW_SHOWNOACTIVATE);
        end;}
          //SendMessage(ParentForm.Handle, WM_ACTIVATE, 0, 0);
          //SendMessage(ParentForm.Handle, WM_NCACTIVATE, Ord(True), 0);
      end;
    end;
  ctParams, ctHint:
    begin
  ...
  if Assigned(F.CurrentEditor) then
    ...
    begin
      ...
      try
        ...
        begin
          if Lines.AccessStringLength(CaretY - 1) > WordEnd.Char then
          begin
            Ln := Lines[CaretY - 1];
            Index := WordEnd.Char;
            while Ln[Index] < #33 do
              Inc(Index);
            if Ln[Index] = Value[Length(Value)] then
              Delete(Value, Length(Value), 1);
          end;
          for Index := 1 to Length(Value) do
            (F.CurrentEditor as TCustomSynEdit).CommandProcessor(ecChar, Value[Index], nil);
          //SelText := Value;
        end;
...
begin
  ...
  fTimerInterval:= 1000;
  fNoNextKey := False;
  fShortCut := 0;//Menus.ShortCut(Ord(' '), [ssCtrl]);
  Options := DefaultProposalOptions;
  fEditors := TList.Create;
end;

procedure TSynCompletionProposal.SetShortCut(Value: TSynShortCut);
begin
  FShortCut := Value;
end;

procedure TSynCompletionProposal.EditorKeyDown(Sender: TObject;
  var Key: Word; Shift: TSynShiftState);
var
  ShortCutKey: Word;
  ShortCutShift: TSynShiftState;
begin
  DisasmSynShortcut(fShortCut, ShortCutKey, ShortCutShift);
    ...
    begin
      ...
      begin
        if DefaultType = ctHint then
          ...
          d := DisplayXY;
          p := ClientToScreen(RowColumnToPixels(DisplayCoord(d.Column, Succ(d.Row))));
        end;
...
begin
  ...
  fNoNextKey := false;
  fShortCut := 0;//Menus.ShortCut(Ord(' '), [ssShift]);
end;

procedure TSynAutoComplete.SetShortCut(Value: TSynShortCut);
begin
  FShortCut := Value;
end;
