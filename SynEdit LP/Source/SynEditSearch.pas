...
function TSynEditSearch.Replace(const aOccurrence, aReplacement: UnicodeString; Index: Integer): UnicodeString;
begin
  Result := aReplacement;
end;                     
...
begin
  ...
  begin
    if CaseSensitive then
      ...
      Origin := PWideChar(WideLowerCase(NewText));
...
function TSynEditSearch.GetReplacePattern: UnicodeString;
begin
  Result := EmptyStr;
end;

procedure TSynEditSearch.SetReplacePattern(const Value: UnicodeString);
begin
  { Do nothing }
end;
