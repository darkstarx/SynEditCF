...
type
  ...
  TSynKeyEvent = procedure(Sender: TObject; var Key: Word;
    Shift: TSynShiftState) of object;
  ...
  public
    ...
    procedure ExecuteKeyDown(Sender: TObject; var Key: Word; Shift: TSynShiftState);
...
procedure TSynEditKbdHandler.AddKeyDownHandler(aHandler: TSynKeyEvent);
begin
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        TSynKeyEvent(Items[idx])(Sender, Key, Shift);
        if (Key = 0) then
        begin
          fInKeyDown := False;
          Exit;
...
begin
  ...
  fInMouseDown := True;
  try
    for cHandler := Pred(fMouseDownChain.Count) downto 0 do
...
begin
  ...
  fInMouseUp := True;
  try
    for cHandler := Pred(fMouseUpChain.Count) downto 0 do
...
procedure TSynEditKbdHandler.RemoveKeyDownHandler(aHandler: TSynKeyEvent);
begin
