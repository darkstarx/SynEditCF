...
uses
  SysUtils, Classes, Windows, Graphics, Math;
...
type
  ...
  public
    ...
    procedure ExtTextOut(X, Y: Integer; Options: TTextOutOptions; ARect: TRect;
      Text: PWideChar; Length: Integer; ETODist: PIntegerArray = nil); virtual;
...
function UniversalTextOut(DC: HDC; X, Y: Integer; Str: PChar;
  Count: Integer): Boolean;
function UniversalExtTextOut(DC: HDC; X, Y: Integer; Options: TTextOutOptions;
  Rect: PRect; Str: PWideChar; Count: Integer; ETODist: PIntegerArray): Boolean;

implementation

uses
  SynUsp10, SynUnicode, SynEditMiscProcs;
...
begin
  ...
  begin
    GlyphBufferSize := (3 * Count) shr 1 + 16;
    Result := Succeeded(ScriptStringAnalyse(DC, Str, Count, GlyphBufferSize, -1,
      SSAnalyseFlags, 0, nil, nil, nil, nil, nil, @SAA));
    Result := Result and Succeeded(ScriptStringOut(SAA, X, Y, 0,
      nil, 0, 0, False));
    Result := Result and Succeeded(ScriptStringFree(@SAA));
  end
  else
    Result := ExtTextOutW(DC, X, Y, 0, nil, Str, Count, nil);
end;
...
begin
  ...
  if Assigned(pCharHeight) then
    pCharHeight^ := Abs(TM.tmHeight) + TM.tmExternalLeading;
end;
...
constructor TheTextDrawer.Create(CalcExtentBaseStyle: TFontStyles;
  BaseFont: TFont);
begin
  inherited Create;
...
procedure TheTextDrawer.ExtTextOut(X, Y: Integer; Options: TTextOutOptions;
  ARect: TRect; Text: PWideChar; Length: Integer; ETODist: PIntegerArray = nil);
var
  EIS: Boolean;
  ...
  begin
    ...
    for i := 0 to Length - 1 do
      FETODist[i] := CharWidthTable(Text[i]) * CharWidth;
  end;
  ...
  begin
    ...
    if EIS then
      ETODist[Length - 1] := Max(RealCharWidth, CharWidth)
    else
...
begin
  ...
  if EIS then
    UniversalExtTextOut(FDC, X, Y, Options, @ARect, Text, Length, ETODist)
  else
