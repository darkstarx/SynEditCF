...
uses
  Windows, Messages, SysUtils, Classes, Controls, Graphics, Themes,
...
begin
 ...
 try
  ...
  if not ParentBackground then
   ...
   FTempBmp.Canvas.Draw(0, 0, PBack);
...
begin
 ...
 Invalidate;
 FOld := k;
 if (o <> k) and Assigned(FOnChange) then FOnChange(Self);
end;
...
begin
 ...
 FTotalCells := FColors.Count - 1;
 if FCheckedIndex > FTotalCells then FCheckedIndex := -1;
 CalcAutoHeight;
 Invalidate;
end;
